...
class TicketProgressDataTable extends DataTable
{
    /**
    ...
    public function query(): QueryBuilder
    {
        if (Auth::user()->roles[0]->name == "engineer") {
            $data = Ticket::with(["category", "owner", "assign_to"])->select("*")->where("status", "progress")->where("assign_id", Auth::user()->id);
        } else if (Auth::user()->roles[0]->name == "kanim") {
            $data = Ticket::with(["category", "owner", "assign_to"])->select("*")->where("status", "progress")->where("owner_id", Auth::user()->id);
        } else {
            $data = Ticket::with(["category", "owner", "assign_to"])->select("*")->where("status", "progress");
        }
