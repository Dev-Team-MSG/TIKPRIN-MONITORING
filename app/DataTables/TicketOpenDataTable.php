...
class TicketOpenDataTable extends DataTable
{
    /**
    ...
    public function query(Ticket $model): QueryBuilder
    {
        if (Auth::user()->roles[0]->name == "engineer") {
            $data = Ticket::with(["category", "owner"])->select("*")->where("status", "open");
        } else if (Auth::user()->roles[0]->name == "kanim") {
            $data = Ticket::with(["category", "owner"])->select("*")->where("status", "open")->where("owner_id", Auth::user()->id);
        } else {
            $data = Ticket::with(["category", "owner"])->select("*")->where("status", "open");
        }
